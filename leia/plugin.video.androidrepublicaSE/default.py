...
from resources.lib.auth import get_mac, get_hotp  # Updated imports
...
source_file = os.path.join(home, 'pkcs7_file')
...
def addon_log(string, level=xbmc.LOGDEBUG):
    if debug == 'true':
        xbmc.log("[plugin.video.androidrepublicaSE-{0}]: {1}".format(addon_version, string), LOGINFO)
    else:
        xbmc.log("[plugin.video.androidrepublicaSE-{0}]: {1}".format(addon_version, string), level)
...
def index():
    ...
    # Check if the PIN file exists and skip validation silently
    if os.path.isfile(test):
        return  # Skip the verification silently if the PIN file exists

    if vauth == 'false':
        DIALOG.ok(addon_name, 
                 f"[COLOR cyan][SE] Sila Hantar ID Code dibawah Kepada Admin: {vmail}[/COLOR] "
                 f"[COLOR lime]{valmac}[/COLOR]")
        
        mess1 = 'Sila Masukkan Pin anda dan Tekan OK!'
        vmess1 = getKeyboard(default="", heading=mess1, hidden=False)
        
        if not vmess1:
            DIALOG.notification(addon_name, "[COLOR cyan]Pin diperlukan[/COLOR]")
            sys.exit()
            
        if get_hotp(valmac, int(1)) == int(vmess1):
            addon.setSetting('vinstall', 'true')
            os.makedirs(os.path.dirname(test), exist_ok=True)
            with open(test, 'w') as f:  # Store the verified PIN
                f.write(vmess1)
            
            # Popup notification after successful verification
            DIALOG.ok(addon_name, 
                      "[COLOR lime]Pengesahan Berjaya![/COLOR]\n\n"
                      "[COLOR yellow]Pin akan kekal selagi add-on tidak didelete. Harap Maklum.[/COLOR]")
            return
        else:
            DIALOG.notification(addon_name, "[COLOR cyan]Salah Pin Bossku. Sila Cuba Lagi[/COLOR]")
            sys.exit()
        ...
        if get_hotp(valmac, int(1)) != int(vread):
            addon.setSetting('vinstall', 'false')
            sys.exit()
        else:
            return
