...
source_file = os.path.join(home, 'pkcs5_file')
...
def addon_log(string, level=xbmc.LOGDEBUG):
    if debug == 'true':
        xbmc.log("[plugin.video.androidrepublica-{0}]: {1}".format(addon_version, string), LOGINFO)
    else:
        xbmc.log("[plugin.video.androidrepublica-{0}]: {1}".format(addon_version, string), level)
...
def getKeyboard( default="", heading="", hidden=False ):
	...
	if keyboard.isConfirmed():
		return keyboard.getText()#unicode( keyboard.getText(), "utf-8" )
	return default
...
def index():
    ...
    try:
        ...
        # Since it's a raw text file, we can directly extract the PIN
        vpost = response.text.strip()
        xbmc.log(f"Extracted PIN: {vpost}", xbmc.LOGINFO)

    except requests.exceptions.RequestException as e:
        xbmc.log(f"Error fetching the PIN: {e}", xbmc.LOGERROR)
        DIALOG.notification(addon_name, "[COLOR red]Failed to retrieve PIN! Please check your internet connection.[/COLOR]")
        sys.exit()

    vpin = addon.getSetting('vpin')

    # Compare the extracted PIN with the one stored in addon settings
    if vpin == '' or vpin != vpost:
        DIALOG.ok(addon_name, "[COLOR cyan]Sila dapatkan pin secara percuma di[/COLOR][COLOR lime] https://t.me/armctv [/COLOR]")

        mess1 = 'Pin juga boleh didapati di [COLOR lime]https://www.facebook.com/armctv[/COLOR]'
        vmess1 = DIALOG.input(mess1, type=xbmcgui.INPUT_ALPHANUM)

        if vmess1.strip() == vpost:
            addon.setSetting('vpin', vmess1)
        else:
            DIALOG.notification(addon_name, "[COLOR cyan]Salah Pin Code tu Bosskuu. Cuba Lagi[/COLOR]")
            sys.exit()
    else:
        xbmc.log(f"PIN matched: {vpin}", xbmc.LOGINFO)
        return
